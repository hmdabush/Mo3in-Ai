        ...
        try {
            const res = await fetch('/api/generate-content', {
                ...
                body: JSON.stringify({
                    ...
                    systemPrompt: `You are a world-class marketing strategist and brand consultant with 20+ years of experience working with Fortune 500 companies and high-growth startups in the MENA region. You specialize in data-driven marketing strategies, competitive analysis, and go-to-market planning. Your analyses are specific, actionable, and backed by real market insights. Always respond in JSON format only with no additional text.`,
    ...
    const handleSectionGenerate = useCallback(async (section: MarketingSection) => {
        ...
        try {
            ...
            const sectionPrompts: Record<string, string> = {
                audit: `Perform a comprehensive marketing audit for ${state.brandName} (${state.industry}). Analyze their digital presence, content strategy, conversion optimization, SEO health, and brand positioning. Return JSON: { "overallScore": number(0-100), "categories": [{ "name": string, "score": number, "weight": string, "findings": [string] }], "topIssues": [string], "quickWins": [string] }. Include 6 categories with 3-4 findings each, 4 top critical issues, and 3 quick wins that can be implemented within a week.`,
                copy: `Generate optimized marketing copy for ${state.brandName} (${state.industry}). Analyze common copywriting mistakes and provide professional rewrites using proven frameworks (AIDA, PAS, BAB). Return JSON: { "headlines": [{ "before": string, "after": string, "framework": string, "improvementNote": string }], "ctas": [{ "before": string, "after": string, "conversionTip": string }], "bodyRewrite": string, "toneGuide": string }. Include 4 headline rewrites, 4 CTA rewrites, and a comprehensive tone guide.`,
                emails: `Create a complete email marketing sequence for ${state.brandName} (${state.industry}). Design a 7-email welcome/nurture sequence with proven open-rate optimization. Return JSON: { "sequenceType": string, "averageOpenRate": string, "emails": [{ "subject": string, "preview": string, "dayNumber": number, "purpose": string, "keyMessage": string, "cta": string }] }. Include 7 emails with compelling subject lines and specific CTAs.`,
                social: `Create a 14-day social media content calendar for ${state.brandName} (${state.industry}). Mix content pillars strategically for maximum engagement. Return JSON: { "strategy": string, "contentPillars": [string], "posts": [{ "day": number, "pillar": string, "platform": string, "hook": string, "contentType": string, "time": string, "hashtags": [string] }] }. Include 14 posts across multiple platforms with specific hooks and optimal posting times for MENA region.`,
                ads: `Create multi-platform ad campaigns for ${state.brandName} (${state.industry}). Design platform-specific ad copy with targeting recommendations. Return JSON: { "totalBudget": string, "platforms": [{ "name": string, "budget": string, "headline": string, "description": string, "cta": string, "targeting": string, "expectedCPM": string, "expectedCTR": string }] }. Include Google, Meta, LinkedIn, TikTok with realistic performance benchmarks.`,
                funnel: `Analyze and optimize the marketing funnel for ${state.brandName} (${state.industry}). Identify conversion bottlenecks and provide specific fixes. Return JSON: { "stages": [{ "name": string, "conversionRate": string, "dropOff": string, "issue": string, "recommendation": string, "expectedImprovement": string }] }. Include 6 stages: Awareness, Interest, Consideration, Intent, Purchase, Retention.`,
                competitors: `Perform competitive intelligence analysis for ${state.brandName} in the ${state.industry} industry. Identify 4 key competitors with detailed analysis. Return JSON: { "marketPosition": string, "competitors": [{ "name": string, "positioning": string, "pricing": string, "strengths": [string], "weaknesses": [string], "marketShare": string }], "opportunities": [string] }. Include 4 competitors with 3 strengths and 3 weaknesses each, plus 4 market opportunities.`,
                landing: `Perform a conversion rate optimization (CRO) audit for ${state.brandName} (${state.industry}) landing page. Use the 7-point CRO framework. Return JSON: { "score": number(0-100), "checks": [{ "area": string, "status": "pass"|"fail"|"warning", "note": string, "fix": string }], "abTestIdeas": [string] }. Include 8 checks covering: headline clarity, CTA visibility, social proof, form optimization, mobile UX, page speed, trust signals, value proposition. Add 3 A/B test ideas.`,
                launch: `Create a detailed 8-week product launch playbook for ${state.brandName} (${state.industry}). Include pre-launch, launch, and post-launch phases. Return JSON: { "launchDate": string, "weeks": [{ "week": number, "phase": string, "focus": string, "tasks": [string], "kpis": [string] }] }. Include 8 weeks with 4-5 tasks each and specific KPIs per week.`,
                proposal: `Create a professional client marketing proposal for ${state.brandName} (${state.industry}). Include executive summary, strategy, pricing, and ROI projections. Return JSON: { "executiveSummary": string, "sections": [{ "title": string, "content": string }], "pricing": [{ "tier": string, "price": string, "features": [string], "bestFor": string }], "roi": { "month3": string, "month6": string, "month12": string } }. Include 4 sections, 3 pricing tiers, and realistic ROI projections.`,
                report: `Generate a comprehensive marketing performance report for ${state.brandName} (${state.industry}). Return JSON: { "period": string, "highlights": [string], "sections": [{ "title": string, "content": string, "metrics": [{ "name": string, "value": string, "trend": string }] }], "recommendations": [string] }. Include 5 sections with 3 metrics each.`,
                seo: `Perform a detailed SEO audit for ${state.brandName} (${state.industry}). Cover on-page, technical, and content SEO. Return JSON: { "score": number(0-100), "onPage": [{ "check": string, "status": "pass"|"fail"|"warning", "detail": string, "priority": string }], "technicalIssues": [{ "issue": string, "impact": string, "fix": string }], "keywordOpportunities": [{ "keyword": string, "volume": string, "difficulty": string, "currentRank": string }] }. Include 10 on-page checks, 5 technical issues, and 5 keyword opportunities.`,
                brand: `Analyze and define brand voice for ${state.brandName} (${state.industry}). Create a comprehensive brand voice framework. Return JSON: { "voiceDimensions": [{ "dimension": string, "score": number(0-100), "description": string, "doExample": string, "dontExample": string }], "toneMap": [{ "context": string, "tone": string, "example": string }], "personality": [{ "trait": string, "description": string }], "vocabulary": { "preferred": [string], "avoid": [string] } }. Include 5 voice dimensions with do/don't examples, 6 tone contexts, 5 personality traits, and vocabulary guide.`,
            };
            ...
            const res = await fetch('/api/generate-content', {
                ...
                body: JSON.stringify({
                    ...
                    systemPrompt: `You are an expert marketing analyst specializing in ${sectionName} for businesses in the MENA and global markets. Generate specific, actionable, data-driven results with real-world benchmarks and practical recommendations. Every suggestion must be immediately implementable. Respond ONLY in valid JSON format with no additional text.`,
                    prompt,
                    maxTokens: 3000,
                }),
            });
